#"""
#Moldified from tensorflow rnn_cell.py
#Gate state are shown when use __Call__ if 'verbose = True' 
#by return a tuple of (input_gate,output_gate,forget_gate)
#
#"""
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or type(self).__name__):  # "GRUCell"
      with vs.variable_scope("Gates"):  # Reset gate and update gate.
        ...
        r, u = array_ops.split(_linear([inputs, state],2 * self._num_units, True, 1.0),2,1)
...
class BasicLSTMCell(RNNCell):
  ...
  def __call__(self, inputs, state, scope=None, verbose = False):
    ...
    with vs.variable_scope(scope or type(self).__name__):  # "BasicLSTMCell"
      ...
      forget_gate = sigmoid(f + self._forget_bias)

      new_c = (c * forget_gate + input_gate *
               self._activation(j))
      ...
      if verbose == False:
        ...
        return new_h, new_state, (input_gate,output_gate,forget_gate)
...
class LSTMCell(RNNCell):
  ...
  def __call__(self, inputs, state, scope=None, verbose = False):
    ...
    with vs.variable_scope(scope or type(self).__name__,
      ...
      if self._use_peepholes:
        forget_gate = sigmoid(f + self._forget_bias + w_f_diag * c_prev)
        input_gate = sigmoid(i + w_i_diag * c_prev)
      else:
        forget_gate = sigmoid(f + self._forget_bias)
        input_gate = sigmoid(i)
      c = (forget_gate * c_prev + input_gate * self._activation(j))
      ...
      if self._use_peepholes:
        output_gate = sigmoid(o + w_o_diag * c)
      else:
        output_gate = sigmoid(o)
      m = output_gate * self._activation(c)
    ...
    if verbose == False:
      ...
      return m, new_state, (input_gate,output_gate,forget_gate)
...
def _linear(args, output_size, bias, bias_start=0.0, scope=None, trainable = True):
  ...
  with vs.variable_scope(scope or "Linear"):
    matrix = vs.get_variable("Matrix", [total_arg_size, output_size],trainable = trainable)
    if len(args) == 1:
    ...
    bias_term = vs.get_variable(
        "Bias", [output_size],
        initializer=init_ops.constant_initializer(bias_start), trainable = trainable)
